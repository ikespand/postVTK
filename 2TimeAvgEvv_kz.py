...
NAX=NAX
Evv = np.zeros((NAX, len(yUnique)-2))
Delete(StartFile)
del StartFile
for file_name in rSTChannelal_vtu:
	...
	for jj in range(1,(len(yUnique)-1)):
		...
		v = npvtk.vtk_to_numpy(dataPlane.GetPointData().GetArray('v'))	
		x=np.array(x)
		y=np.array(y)
		z=np.array(z)
		Nx=len(np.unique(z))
		Nz=len(v)/Nx #len(np.unique(x))
		...
		raw=  x,y,z,vFluc # Span, WallNormal, Stream, VelFluc
	...
	del VTUFILE
print("***********************************************************")
Evv=Evv/float(len(rSTChannelal_vtu)) # Average of Evv over the time. For kx, not necessary.
...
np.savetxt(EuuName,Evv, delimiter=',')
np.savetxt(yName,yS, delimiter=',')
#np.savetxt(OutFile+'Evv.csv',Evv, delimiter=',')
